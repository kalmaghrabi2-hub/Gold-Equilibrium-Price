#!/usr/bin/env python3
"""Benchmark-aware weekly gold fair-value calibration.

Forecast target: next observed weekly COMEX gold close (GC=F).
Model family: ridge ARX on the prior week's gold level and macro factors.
...
TARGET = "GC=F"
MACRO = ["DX-Y.NYB", "^TNX", "^VIX", "TIP"]
SERIES = ["LAG_GOLD_LOG"] + MACRO
START = 1262304000
MIN_TRAIN = 156
...
def main():
    gold, gold_url = chart(TARGET)
    factors, urls = {}, {}
    for symbol in MACRO:
        factors[symbol], urls[symbol] = chart(symbol)

    common = sorted(set(gold).intersection(*[set(factors[s]) for s in MACRO]))
    raw = [(w, gold[w], [factors[s][w] for s in MACRO]) for w in common]
    rows = []
    for i in range(1, len(raw)):
        prev_week, prev_gold, prev_macro = raw[i - 1]
        week, current_gold, _ = raw[i]
        if week - prev_week == 1:
            rows.append((week, current_gold, [math.log(prev_gold)] + prev_macro))
    ...
    last_week, last_gold, last_macro = raw[-1]
    live_values = [math.log(last_gold)] + last_macro
    live_x = [1.0] + [(v - m) / s for v, m, s in zip(live_values, means, sds)]
    raw_live = math.exp(sum(a * b for a, b in zip(live_x, beta)))
    live_value = blended(raw_live, last_gold, weight)
    ...
    output = {
        ...
        "model": "gold-weekly-benchmark-aware-ridge-v3",
        "frequency": "weekly",
        "forecast_horizon_weeks": 1,
        "target": TARGET,
        "target_source": gold_url,
        "series": SERIES,
        "predictor_sources": {"LAG_GOLD_LOG": gold_url, **urls},
        "beta": beta,
        "means": means,
        "sds": sds,
        "ridge_lambda": lam,
        ...
        "live": {
            ...
            "anchor_usd_oz": round(last_gold, 4),
            "raw_model_fair_value_usd_oz": round(raw_live, 4),
            "fair_value_usd_oz": round(live_value, 4),
            "predicted_return_pct": round((live_value / last_gold - 1.0) * 100.0, 3),
        },
        "rules": {
