...
def main():
    ...
    if len(obs) < MIN_TRAIN + 1:
        raise RuntimeError(f'insufficient exact WGC observations: {len(obs)}')
    ...
    statistical_checks = {
        'min_oos_quarters': len(aa) >= MIN_OOS,
        'min_mse_skill': skill >= MIN_MSE_SKILL,
        'min_relative_mape_improvement': rel_mape >= MIN_REL_MAPE_IMPROVEMENT,
        'directional_information': direction >= MIN_DIRECTION,
        'sanity_mape': mape <= 12.0,
    }
    statistical_pass = all(statistical_checks.values())
    # WGC historical pages are not yet mapped to exact original publication vintages for every quarter.
    # Therefore this layer cannot be promoted even if descriptive statistical thresholds happen to pass.
    ...
    out = {
        ...
        'physical_price_validation_gate':physical_price_validation_gate, 'walk_forward_gate':'FAIL', 'publication_status':'RESEARCH_ONLY', 'promotion_eligible':False,
        ...
        'rules':{'publication_lag':'features from q may predict q+1 only after verified original release date','missing_data':'exact observations only; no interpolation or imputation','min_training_quarters':MIN_TRAIN,'min_oos_quarters':MIN_OOS,'min_mse_skill_pct':MIN_MSE_SKILL,'min_relative_mape_improvement_pct':MIN_REL_MAPE_IMPROVEMENT,'min_direction_accuracy_pct':MIN_DIRECTION,'no_price_clipping':True}
    }
