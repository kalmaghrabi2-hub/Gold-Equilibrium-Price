#!/usr/bin/env python3
"""Collect auditable institutional macro inputs for gold research.

This is a research/data layer only. It does not change the published price.
Each run records the latest available observation for selected FRED series so a
forward point-in-time archive can be accumulated without retroactive guessing.
"""
...
OUT = ROOT / "docs/gold/data/institutional_inputs.json"
UA = "Mozilla/5.0 GoldEquilibriumInstitutionalCollector/1.0"

SERIES = {
    "real_yield_10y": {"id": "DFII10", "source": "Federal Reserve/FRED", "role": "real yield / monetary opportunity cost"},
    ...
    "nominal_10y": {"id": "DGS10", "source": "Federal Reserve/FRED", "role": "nominal rate control"},
}
